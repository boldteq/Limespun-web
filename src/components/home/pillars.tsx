    ...
    <>
      ...
      <section aria-labelledby="replaces-heading" className="bg-canvas pb-20">
        <div className="mx-auto max-w-[1280px] px-5 sm:px-8">
          <div className="flex flex-col gap-5 rounded-[24px] bg-canvas-deep px-6 py-7 sm:px-10 lg:flex-row lg:items-center lg:gap-12">
            <h2 id="replaces-heading" className="shrink-0 text-[16px] leading-snug font-semibold text-graphite lg:max-w-[240px]">
              One place for the seven apps you open before 10:30
            </h2>
            <span className="hidden h-12 w-px bg-hair-strong lg:block" aria-hidden="true" />
            <ul className="flex flex-wrap items-center gap-x-7 gap-y-3">
              {replaced.map((name) => (
                <li
                  key={name}
                  className="text-[17px] font-semibold tracking-[-0.01em] text-graphite/45 line-through decoration-ember/70 decoration-2"
                >
                  {name}
                </li>
              ))}
      ...
      <section className="bg-white py-24 sm:py-28">
        <div className="mx-auto grid max-w-[1280px] gap-14 px-5 sm:px-8 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.5fr)] lg:gap-20">
          <div className="lg:sticky lg:top-32 lg:self-start">
            ...
            <p className="mt-6 max-w-[400px] text-[17px] leading-[1.6] text-mute">
          ...
          <div className="grid gap-px overflow-hidden rounded-[24px] bg-hair ring-1 ring-hair sm:grid-cols-2">
            {pillars.map((p) => (
              <article key={p.title} className="flex flex-col bg-white p-8 sm:p-10">
                {p.art}
                <h3 className="mt-7 text-[26px] font-medium tracking-[-0.01em] text-graphite">{p.title}</h3>
