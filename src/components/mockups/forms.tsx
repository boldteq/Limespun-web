 ...
 * fields, Auto-sent / Required chips, uses, Edit), the Submissions tab
 * (Signed · Sent · Expired) and the live front-desk kiosk ("Kiosk is live"),
 * which covers the whole app as it does in the product.
 * ConsentSignPhone is Elena R. on her phone: the allergy question answered,
 * her signature, signed at 9:42.
 */
...
/** Owen P.'s medical history link, sent Tue, Sep 29, lapsed unsigned after seven days. */
const EXPIRED: { client: string; form: string; session: string; status: SubStatus; when: string; via: Submission["via"] } = {
  client: "Owen P.",
...
const SUB_ROWS: { client: string; form: string; session: string; status: SubStatus; when: string; via: Submission["via"] }[] = [...SUBMISSIONS, EXPIRED];
...
function SubmissionsCard() {
  const statuses: SubStatus[] = ["Signed", "Sent", "Expired"];
  const rows: AppTableRow[] = SUB_ROWS.map((s) => ({
    ...
    cells: [
      ...
      <span key="d" className="flex flex-col leading-tight">
        ...
        <span className="text-ui-xs text-app-mute">{s.status === "Signed" ? `on ${s.via === "Kiosk" ? "the kiosk" : s.via === "Phone" ? "their phone" : "the link"}` : "by link"}</span>
      </span>,
    ],
  }));
  return (
    <div className="overflow-hidden rounded-app-lg bg-app-surface ring-1 ring-app-border">
      <div className="flex flex-wrap items-center gap-1.5 border-b border-app-border px-4 py-3">
        <span className="inline-flex h-6 items-center rounded-full bg-app-text px-2.5 text-ui-xs font-semibold text-white">All {SUB_ROWS.length}</span>
        {statuses.map((st) => (
          <span key={st} className="inline-flex h-6 items-center gap-1 rounded-full bg-graphite/[0.06] px-2.5 text-ui-xs font-semibold text-app-soft">
            {st}
            <span className="tabular-nums">{SUB_ROWS.filter((r) => r.status === st).length}</span>
          </span>
        ))}
      </div>
      ...
      <div className="divide-y divide-app-border @xl:hidden">
        {SUB_ROWS.map((s) => (
          <div key={`${s.client}-${s.form}`} className="flex items-center gap-2.5 px-3.5 py-2.5">
            <AppAvatar initials={initials(s.client)} size="sm" />
...
export function FormsScreen({ tab = "templates", className }: { tab?: FormsTab; className?: string }) {
  ...
  return (
    <AppFrame
      ...
      actions={
        <>
          <AppButton icon={Tablet}>Kiosk mode</AppButton>
    ...
    >
      <AppTabs
        tabs={[
          ...
          { label: "Submissions", count: SUB_ROWS.length },
        ]}
        ...
        {state === "form" ? (
          <div className="flex flex-1 flex-col gap-2 px-3.5 pt-3 pb-4">
            <div className="flex items-center gap-1">
              {[0, 1, 2, 3, 4, 5].map((i) => (
                <span key={i} className="h-1 flex-1 rounded-full bg-app-text" />
              ))}
            </div>
            ...
            <div className="rounded-app px-2.5 py-2 ring-1 ring-app-border">
              ...
              <p className="mt-0.5 text-[11px] leading-snug font-semibold text-app-text">
                Red ink. I reacted after session 1 in August.
              </p>
            </div>
