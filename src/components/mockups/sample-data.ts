 ...
 * - AI never draws: moodboard references are the client's own uploads and the
 *   brief is text the artist edits.
 */
...
export const TODAY_SESSIONS: TodaySession[] = [
  ...
  },
  {
    ...
    endMin: 900,
    client: "Priya S.",
    piece: "Fine-line florals",
    detail: "Fine-line florals · $80 deposit applied today",
    kind: "session",
    status: "confirmed",
    consent: "not-signed",
...
/** The studio's cancellation policy: cancel inside this window and the deposit is kept. */
...
export const DEPOSITS: DepositRow[] = [
  ...
  { client: "Priya S.", artist: "mara", booking: "Fine-line florals · Thu, Oct 8, 1:00", cents: 8000, state: "Applied", note: "Applied to today's session", today: true },
  {
    client: "Owen P.",
    artist: "dev",
  ...
  { client: "Leo B.", artist: "mara", booking: "Chest panel, session 1 · Fri, Oct 9, 11:00 · cancelled", cents: 15000, state: "Kept", note: "Late cancel, inside 48 hours" },
...
/**
 * Leo B.'s late cancel: the booking policy kept the deposit. He cancelled Wed
 * 6:12 PM for Fri 11:00, 40 hours out, so inside the 48-hour window, and before
 * now (Thu 10:40). The freed slot is offered to Nadia H. on the waitlist.
 */
export const LATE_CANCEL = {
  ...
  cents: 15000,
  policy: `Cancel inside ${CANCEL_WINDOW_HOURS} hours: deposit kept`,
} as const;
...
export interface Transaction {
  when: string;
  client: string;
  type: "Deposit" | "Session" | "Walk-in" | "Late cancel";
  artist: ArtistId;
  cents: number;
...
export const TRANSACTIONS: Transaction[] = [
  { when: "Today, 8:05 AM", client: "Kira N.", type: "Deposit", artist: "mara", cents: 15000, status: "Paid", method: "Card" },
  { when: "Today, 7:12 AM", client: "Bea L.", type: "Deposit", artist: "dev", cents: 10000, status: "Paid", method: "Card" },
  { when: "Wed, Oct 7, 6:12 PM", client: "Leo B.", type: "Late cancel", artist: "mara", cents: 15000, status: "Kept", method: "Card" },
  { when: "Wed, Oct 7, 4:40 PM", client: "Bea L.", type: "Session", artist: "dev", cents: 50000, status: "Paid", method: "Card" },
  { when: "Tue, Oct 6, 5:20 PM", client: "Tomás V.", type: "Session", artist: "dev", cents: 90000, status: "Paid", method: "Card" },
  { when: "Tue, Oct 6, 3:05 PM", client: "Walk-ins (4)", type: "Walk-in", artist: "rio", cents: 48000, status: "Paid", method: "Card" },
];

/** Revenue KPI on Today: payments taken today so far ($250). */
export const REVENUE_TODAY_CENTS = sum(TRANSACTIONS.filter((t) => t.when.startsWith("Today")).map((t) => t.cents));
/** Yesterday's payments ($500: Bea's session balance after $100 from her deposit pool). */
export const REVENUE_YESTERDAY_CENTS = sum(
  TRANSACTIONS.filter((t) => t.when.startsWith("Wed, Oct 7") && t.status === "Paid").map((t) => t.cents),
);

/** Commission records this week awaiting approval (Payments › Commissions). */
...
export const COMMISSIONS: CommissionRow[] = [
  ...
  { artist: "rio", client: "Walk-ins (4)", when: "Tue, Oct 6", serviceCents: 48000, artistCents: 33600, status: "Pending approval" },
  { artist: "dev", client: "Bea L.", when: "Wed, Oct 7", serviceCents: 60000, artistCents: 36000, status: "Pending approval" },
];

/** "Commissions owed" KPI: $1,236 across 3 pending approval. */
...
export const PROJECTS: Project[] = [
  {
    ...
    // $300 paid in Thu, Aug 13 → $60 applied to session 3 → $240 held for sessions 4–5.
...
export interface AttentionItem {
  kind: "deposit" | "consent" | "allergy" | "stock";
  title: string;
  detail: string;
  when: string;
  action: string;
...
export const NEEDS_ATTENTION: AttentionItem[] = [
  { kind: "consent", title: "Consent not signed", detail: "Priya S. · Fine-line florals with Mara", when: "Today, 1:00", action: "Send form", tone: "warning" },
  { kind: "allergy", title: "Red ink allergy", detail: "Elena R. · Back piece, session 2 with Dev", when: "Today, 1:30", action: "Open client", tone: "danger" },
  { kind: "deposit", title: "Deposit pending", detail: "Owen P. · $150 requested Wed, Oct 7", when: "Sat, Oct 10, 1:00", action: "Send reminder", tone: "warning" },
  { kind: "stock", title: "Low stock", detail: "Black lining ink, 8 oz · 1 left, reorder at 2", when: "Inventory", action: "Create PO", tone: "neutral" },
];
...
/** Template names from forms/_proto/lib.tsx. */
export const FORM_TEMPLATES: { name: string; kind: string; submissions30d: number }[] = [
  { name: "Tattoo consent — general", kind: "Consent", submissions30d: 86 },
  { name: "Medical history", kind: "Intake", submissions30d: 41 },
  { name: "Aftercare acknowledgement", kind: "Aftercare", submissions30d: 79 },
  { name: "Touch-up waiver", kind: "Waiver", submissions30d: 12 },
  { name: "Minor / guardian consent", kind: "Consent", submissions30d: 3 },
...
export const SUBMISSIONS: Submission[] = [
  ...
  { client: "Priya S.", form: "Tattoo consent — general", session: "Fine-line florals", status: "Sent", when: "Sent Tue, Oct 6", via: "Link" },
...
export const SEGMENTS: { label: string; description: string; clients: number }[] = [
  ...
  { label: "New (last 30 days)", description: "Joined in the last 30 days", clients: 23 },
...
export const CAMPAIGNS: Campaign[] = [
  ...
  { name: "Friday the 13th flash", channel: "SMS", audience: "All clients", recipients: 412, status: "Scheduled", when: "Thu, Nov 5" },
  { name: "Openings this month", channel: "Email", audience: "Inactive 90 days", recipients: 96, status: "Draft", when: "—" },
];
...
/** Flash library. Tiles show the label ("F1"), never drawn art. */
export const FLASH: FlashPiece[] = [
  { label: "F1", title: "Dagger and rose", artist: "rio", priceCents: 18000, status: "Available" },
  { label: "F2", title: "Moth", artist: "mara", priceCents: 14000, status: "Reserved" },
  { label: "F3", title: "Snake coil", artist: "rio", priceCents: 22000, status: "Available" },
  { label: "F4", title: "Peony sprig", artist: "mara", priceCents: 12000, status: "Available" },
  { label: "F5", title: "Swallow pair", artist: "rio", priceCents: 15000, status: "Available" },
  { label: "F6", title: "Wave crest", artist: "dev", priceCents: 20000, status: "Reserved" },
];
...
export const PORTFOLIO: PortfolioPiece[] = [
  { label: "P1", title: "Koi sleeve, session 3", artist: "dev", style: "Japanese", healed: true, featured: true },
...
/** Drafted brief the artist edits. References are Jo's own uploads. */
...
export const LOCATIONS: LocationRow[] = [
  {
    ...
    bookings: 54,
    revenueCents: sum(WEEKLY_REVENUE.slice(-4).map(weekTotalCents)),
    note: "Main studio",
  },
