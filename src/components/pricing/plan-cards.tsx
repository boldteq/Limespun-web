...
const OPTIONS: { key: BillingOption; label: string; chip?: string }[] = [
  { key: "monthly", label: "Monthly" },
  { key: "annual", label: "Annual", chip: `−${ANNUAL_DISCOUNT_PERCENT}%` },
  ...(FOUNDING_OFFER_OPEN ? [{ key: "lifetime" as const, label: "Lifetime", chip: "Founding" }] : []),
];
...
function BillingToggle({ value, onChange }: { value: BillingOption; onChange: (v: BillingOption) => void }) {
  return (
    <div className="flex flex-col items-center gap-3">
      <div role="radiogroup" aria-label="Billing" className="inline-flex rounded-full bg-canvas p-1 ring-1 ring-hair">
        {OPTIONS.map((o) => {
          ...
          return (
            <button
              ...
              className={cn(
                "inline-flex h-10 items-center gap-2 rounded-full px-4 text-[14px] font-semibold transition-colors duration-150 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-graphite sm:px-5",
...
function PlanCard({ plan, option }: { plan: Plan; option: BillingOption }) {
  ...
  return (
    ...
    >
      {plan.recommended && (
        <span className="absolute -top-3 left-6 rounded-full bg-ember px-3 py-1 text-[12px] font-semibold text-white">
          Recommended
        </span>
      )}
      ...
      {plan.recommended ? (
        <PrimaryButton href={ACCOUNT.signUpHref} className="mt-6 w-full">
          {cta}
        </PrimaryButton>
      ) : (
        <SecondaryButton href={ACCOUNT.signUpHref} className="mt-6 w-full">
          {cta}
        </SecondaryButton>
      )}

      <dl className="mt-6 divide-y divide-hair rounded-[14px] bg-canvas px-4 py-1">
        {PLAN_CAPS.map((c) => (
    ...
    <div>
      ...
      <div className="mt-6 grid gap-8 rounded-[20px] bg-canvas p-6 sm:p-8 lg:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)] lg:gap-12">
        ...
        <div className="flex flex-col gap-4 border-t border-hair pt-6 lg:border-t-0 lg:border-l lg:pt-0 lg:pl-12">
          <p className="flex gap-3 text-[15px] leading-snug text-graphite-soft">
